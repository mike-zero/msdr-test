...
while (sysread(F, $buf, $bufsize)) {
	foreach my $s (0..(length($buf)/2-1)) {
		...
		my $avg_ast = $avg_sum >> (AVG_FACTOR_BITS()+SCALE_BITS()) <= $ast_lim ? $avg_sum >> (AVG_FACTOR_BITS()+SCALE_BITS()) : $ast_lim;
		printf "%3d %3d amp=%6u avg=%6u d=%5d avgd=%5d %-${ast_lim}s %-${ast_lim}s\n", $i, $q, $amp, $avg_sum >> AVG_FACTOR_BITS(), $delta, $avg_delta >> AVG_FACTOR_BITS(), '*' x $amp_ast, '*' x $avg_ast;
		$prev_amp = $amp;
		$prev_delta = $delta;
