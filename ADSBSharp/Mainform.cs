...
namespace ADSBSharp
{
    ...
    {        
        ...
        {
            get { return _tickTotal; }
            set { _tickTotal = value; }
        }
        */
        public MainForm()
        {
            ...
            Text = "ADSB# m0 v" + Assembly.GetExecutingAssembly().GetName().Version;
        ...
        private void rtl_SamplesAvailable(object sender, Complex* buf, int length)
        {
            ...
            {
                var mag = buf[i].Real * buf[i].Real + buf[i].Imag * buf[i].Imag;

                if (stateText.Length < 1024)
                {
